...
public class KahluaSdk implements Sdk, ApplicationComponent {
    ...
    public void initComponent() {
		SdkTable pjt = SdkTable.getInstance();
        mySdk = pjt.findSdk(KahluaSdk.NAME);
...
//        }
        ...
        if (mySdk == null) {
            ...
            ApplicationManager.getApplication().runWriteAction(new Runnable() {
                @Override
                public void run() {
					SdkTable.getInstance().addSdk(mySdk);
                }
            });
        } else {
    ...
    private static Sdk createMockSdk(String jdkHome, final String versionName) {
        ...
        final Sdk jdk = new SdkImpl(versionName, LuaSdkType.getInstance());
