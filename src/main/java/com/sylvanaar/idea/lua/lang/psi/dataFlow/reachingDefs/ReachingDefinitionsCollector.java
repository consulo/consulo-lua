...
public class ReachingDefinitionsCollector
{
	...
	public static FragmentVariableInfos obtainVariableFlowInformation(final LuaStatementElement first, final LuaStatementElement last)
	{
		...
		final DFAEngine<IntObjectMap<IntSet>> engine = new DFAEngine<IntObjectMap<IntSet>>(flow, dfaInstance, lattice);
		final IntObjectMap<IntSet> dfaResult = postprocess(engine.performDFA(), flow, dfaInstance);
	...
	//  private static void addClosureUsages(final Map<String, VariableInfo> imap, final Map<String, VariableInfo> omap, final LuaStatement first, final LuaStatement last, LuaControlFlowOwner
	// flowOwner) {
	...
	//      }
	//
	//      private void addUsagesInClosure(final Map<String, VariableInfo> imap, final Map<String, VariableInfo> omap, final LuaClosableBlock closure, final LuaStatement first, final LuaStatement
	// last) {
	...
	private static boolean allProperDefsInFragment(int[] defs, int ref, LinkedHashSet<Integer> fragmentInstructions, int[] postorder)
	{
		for(int def : defs)
		{
			if(!fragmentInstructions.contains(def) && postorder[def] < postorder[ref])
			{
				return false;
			}
		}
	...
	private static LuaType getType(PsiElement element)
	{
		...
		{
			return ((LuaExpression) element).getLuaType();
		}
	...
	private static LinkedHashSet<Integer> getReachable(final LinkedHashSet<Integer> fragmentInsns, final Instruction[] flow, IntObjectMap<IntSet> dfaResult, final int[] postorder)
	{
		final LinkedHashSet<Integer> result = new LinkedHashSet<Integer>();
		for(Instruction insn : flow)
		{
					...
					!((ReadWriteVariableInstruction) insn).isWrite())
			{
				...
				IntSet defs = dfaResult.get(ref);
				PrimitiveIterator.OfInt iterator = defs.iterator();
				while(iterator.hasNext())
				{
					int def = iterator.nextInt();
	...
	private static IntObjectMap<IntSet> postprocess(final ArrayList<IntObjectMap<IntSet>> dfaResult, Instruction[] flow, ReachingDefinitionsDfaInstance dfaInstance)
	{
		...
		for(int i = 0; i < flow.length; i++)
		{
			...
			if(insn instanceof ReadWriteVariableInstruction)
			{
				...
				if(!rwInsn.isWrite())
				{
					...
					IntSet defs = dfaResult.get(i).get(idx);
					if(defs == null)
					{
